<template>
  <div>
    ...
    <div class="flex flex-col md:flex-row gap-4 md:justify-between md:items-center mt-4 md:mt-6">
      <NuxtLink :to="isWebsite ? '/dashboard' : '/dashboard'" target="_blank" class="w-full max-w-[17.5rem] relative">
        <PrimaryCTA :label="isWebsite ? 'Call via healing119.id' : 'Chat via Whatsapp'" :is-width-parent="true">
          <template v-if="isWebsite" #left-icon>
            <IcPhoneWhite />
          </template>
        </PrimaryCTA>
      </NuxtLink>
      <NuxtLink :to="isWebsite ? '/consul' : '/consul'" class="flex items-center gap-2 self-end md:self-auto">
